	...
	*/
	IF( _tipo = 2 ) THEN
		...
		IF(_id_componente = 1) THEN
			...
			LEFT JOIN tramites_licencias_asignaciones_tf licencia_tf1 ON licencia_tf1.id_tramite_licencia_asignacion_tf = atfb1.id_tramites_licencias_asignaciones_tf AND licencia_tf1.id_componente = _id_componente
			...
			WHERE
				...
				COALESCE(emp_tf1.paterno||emp_tf1.materno||emp_tf1.nombre,'')
				= (
					SELECT 
					...
					LEFT JOIN tramites_licencias_asignaciones_tf licencia_tf2 ON licencia_tf2.id_tramite_licencia_asignacion_tf = atfb2.id_tramites_licencias_asignaciones_tf AND licencia_tf2.id_componente = _id_componente
					LEFT JOIN tramites_licencias licencia4 ON licencia4.id_tramite_licencia = licencia_tf2.id_tramite_licencia
					LEFT JOIN empleados emp_tf2 ON emp_tf2.id_empleado = licencia4.id_empleado
		...
		IF (_id_componente = 4) THEN
			...
			FROM asignacion_tiempo_fijo_paraescolares atfb1
			LEFT JOIN tramites_licencias_asignaciones licencia_asign1 ON licencia_asign1.id_tramite_licencia_asignacion = atfb1.id_tramites_licencias_asignaciones AND licencia_asign1.id_componente = id_componente
			LEFT JOIN tramites_licencias licencia1 ON licencia1.id_tramite_licencia = licencia_asign1.id_tramite_licencia
			LEFT JOIN empleados emp1 ON emp1.id_empleado = licencia1.id_empleado
			LEFT JOIN empleados emp_asign ON emp_asign.id_empleado = atfb1.id_empleado
			LEFT JOIN codigos_postales cp1 ON cp1.id_cp = emp_asign.id_cp
			LEFT JOIN cat_categorias_padre catego ON catego.id_cat_categoria_padre = atfb1.id_cat_categoria_padre
			LEFT JOIN cat_materias_paraescolares mat ON mat.id_paraescolar = atfb1.id_cat_materias_paraescolares
			LEFT JOIN cat_tipo_movimiento_personal mov ON mov.id_tipo_movimiento_personal = atfb1.id_tipo_movimiento_personal
			LEFT JOIN cat_tipo_movimiento_personal mov1 ON mov1.id_tipo_movimiento_personal = licencia1.id_movimiento_titular
			LEFT JOIN subprogramas subp ON subp.id_subprograma = atfb1.id_subprograma
			LEFT JOIN claves_sep sep ON sep.id_sep = subp.id_sep
			LEFT JOIN grupos_paraescolares gpos ON atfb1.id_grupo_paraescolares = gpos.id_grupo_paraescolar
			LEFT JOIN tramites_licencias_asignaciones_tf licencia_tf1 ON licencia_tf1.id_tramite_licencia_asignacion_tf = atfb1.id_tramites_licencias_asignaciones_tf AND licencia_tf1.id_componente = _id_componente
			LEFT JOIN tramites_licencias licencia3 ON licencia3.id_tramite_licencia = licencia_tf1.id_tramite_licencia
			LEFT JOIN empleados emp_tf1 ON emp_tf1.id_empleado = licencia3.id_empleado
			LEFT JOIN cat_tipo_movimiento_personal mov2 ON mov2.id_tipo_movimiento_personal = licencia3.id_movimiento_titular
			WHERE
				CAST(COALESCE(atfb1.id_empleado,'0') AS text)||
				CAST(COALESCE(atfb1.id_cat_materias_paraescolares,'0') AS text)||
				CAST(COALESCE(atfb1.id_subprograma,'0') AS text)||
				CAST(COALESCE(atfb1.id_estructura_ocupacional,'0') AS text)||
				COALESCE(emp1.paterno||emp1.materno||emp1.nombre,'')||
				COALESCE(emp_tf1.paterno||emp_tf1.materno||emp_tf1.nombre,'')
				= (
					...
					LEFT JOIN tramites_licencias licencia4 ON licencia4.id_tramite_licencia = licencia_tf2.id_tramite_licencia
					LEFT JOIN empleados emp_tf2 ON emp_tf2.id_empleado = licencia4.id_empleado
					WHERE atfb2.id_asignacion_tiempo_fijo_paraescolares = _id_asignacion 
				)
			GROUP BY 
				emp_asign.filiacion,
				emp_asign.paterno,
	...
	ELSE
		...
		IF (_id_componente = 4) THEN
			...
			LEFT JOIN cat_tipo_movimiento_personal mov2 ON mov2.id_tipo_movimiento_personal = licencia3.id_movimiento_titular
			WHERE
				atfb1.id_asignacion_tiempo_fijo_paraescolares = _id_asignacion
			GROUP BY 
				emp_asign.filiacion,
				emp_asign.paterno,
				emp_asign.materno,
