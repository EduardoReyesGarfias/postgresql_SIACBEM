	...
	*/
	IF( _tipo = 2 ) THEN
		...
		IF(_id_componente = 1) THEN
			...
			INNER JOIN cat_tipo_movimiento_personal mov1 ON mov1.id_tipo_movimiento_personal = licencia1.id_movimiento_titular
			...
			INNER JOIN grupos_estructura_base gpos ON atfb1.id_grupo_estructura_base = gpos.id_grupo_estructura_base
			WHERE
				CAST(atfb1.id_empleado AS text)||
				CAST(atfb1.id_detalle_materia AS text)||
				CAST(atfb1.id_subprograma AS text)||
				CAST(atfb1.id_estructura_ocupacional AS text)||
				emp1.paterno||emp1.materno||emp1.nombre
				= (
					SELECT 
						CAST(atfb2.id_empleado AS text)||
						CAST(atfb2.id_detalle_materia AS text)||
						CAST(atfb2.id_subprograma AS text)||
						CAST(atfb2.id_estructura_ocupacional AS text)||
						emp2.paterno||emp2.materno||emp2.nombre
