CREATE OR REPLACE FUNCTION planteles_hrs_vacantes(id_subprograma int,id_estructura int)
RETURNS TABLE(
	semestre smallint,
...
BEGIN
	...
	(
		...
		sum(i.hora_semana_mes) as hora_semana_mes
		,j.materia
		,(sum(i.hora_semana_mes)-(
	...
	(
		...
		WHERE e.id_estructura_ocupacional = $2 and e.id_subprograma = $1
		GROUP BY a.nombre
		ORDER BY a.nombre
	);
...
select * from planteles_hrs_vacantes(35,26);

SELECT * 
FROM planteles_hrs_vacantes(35,26)
ORDER BY semestre,materia


--select * from subprogramas order by nombre_subprograma